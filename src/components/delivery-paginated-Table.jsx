...
export default function Table({ data }) {
  ...
  const handleRowClick = (user) => {
    navigate('/dashboard/view-delivery-partner', { state: { user } });
  };
  ...
  return (
    <div className='flex flex-col gap-[2.5rem]'>
        ...
        <table className='w-full'>
          ...
          <tbody>
            ...
            {paginatedData.length === 0 && (
              <tr>
                <td colSpan={columns.length} className='text-center px-[1rem] py-[0.62rem]'>
                  <p className='text-[#4B465C] text-[1.125rem] font-[400]'>No delivery partners available</p>
                </td>
              </tr>
      ...
      {paginatedData.length > 0 && <Stack spacing={2} direction="row" justifyContent="center">
        <Pagination count={count} page={currentPage} onChange={handleChangePage} />
      </Stack>}
     {paginatedData.length > 0 && <div className='w-full flex justify-between items-center'>
